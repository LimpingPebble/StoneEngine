...
GLint convert(Core::Image::Channel channel) {
	switch (channel) {
...
GLint convert(Scene::TextureWrap wrap) {
	switch (wrap) {
...
Texture::Texture(Scene::Texture &texture, const std::shared_ptr<OpenGLRenderer> &renderer)
	...
	const auto &imageSource = texture.getImage();
	...
	GLint format = convert(imageData->getChannels());
